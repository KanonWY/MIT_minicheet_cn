...
void ConvexMPCLocomotion::run(ControlFSMData<float>& data)
{
    ...
    for (int foot = 0; foot < 4; foot++)
    {
        ...
        if (swingState > 0)  // foot is in swing
        {
            ...
            //                                          hw_i->leg_controller->leg_datas[foot].qd, 0); // velocity dependent friction
            //                                          compensation todo removed
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int* mpcTable, ControlFSMData<float>& data, bool omniMode)
{
    ...
    if ((iterationCounter % iterationsBetweenMPC) == 0)
    {
        ...
        if (current_gait == 4)
        {
            float trajInitial[12] = { _roll_des, _pitch_des /*-hw_i->state_estimator->se_ground_pitch*/,
                ...
                (float)_body_height /*fsm->main_control_settings.p_des[2]*/, 0, 0, 0, 0, 0, 0 };
