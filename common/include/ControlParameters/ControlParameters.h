...
/**
 * @brief 控制参数的值指针，使用联合体
 * @note 后续考虑使用 variant 来改进
 */
...
/**
 * @brief 使用 map 管理所有的控制参数
 */
class ControlParameterCollection
{
public:
    explicit ControlParameterCollection(const std::string& name)
        : _name(name)
    {
    }
    ...
    std::map<std::string, ControlParameter*> _map;  ///<<< 控制参数的 map
...
/**
 * @brief 单个控制参数的抽象
 * @note  注意该类不实际拥有一个控制参数的值，而是指向一个存在的值
 */
class ControlParameter
{
public:
    ...
    bool                      _set = false;
    ControlParameterValuePtr  _value;  ///<<< 控制参数值指针
    std::string               _name;
    std::string               _units;
