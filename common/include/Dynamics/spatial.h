...
namespace spatial
{
    ...
    /**
     * @brief 关节类型枚举
     * @note https://robot.czxy.com/docs/kinematics/urdf/joint/
     */
    enum class JointType
    {
        Prismatic,     ///<<< 滑动关节。可以平移
        Revolute,      ///<<< 旋转关节。负责旋转的，有旋转角度的限制.
        FloatingBase,  ///<<< 悬浮关节。具备六个自由度，3T3R
        Nothing
    };
     ...
     */
    /**
     * @brief 计算从 A 到 B 的空间坐标变换，其中 B 绕轴旋转 θ
     */
    template <typename T>
    ...
    auto forceCrossMatrix(const Eigen::MatrixBase<T>& v)
    {
        ...
        f << 0, -v(2), v(1), 0, -v(5), v(4), v(2), 0, -v(0), v(5), 0, -v(3), -v(1), v(0), 0, -v(4), v(3), 0, 0, 0, 0, 0, -v(2), v(1), 0, 0,
