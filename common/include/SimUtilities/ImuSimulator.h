...
public:
    explicit ImuSimulator(SimulatorControlParameters& simSettings, u64 seed = 0)
        : _simSettings(simSettings),
          ...
          _vectornavAccelerometerDistribution(
              -simSettings.vectornav_imu_accelerometer_noise, simSettings.vectornav_imu_accelerometer_noise),
    ...
    void computeAcceleration(const FBModelState<T>& robotState, const FBModelStateDerivative<T>& robotStateD, Vec3<float>& acc,
