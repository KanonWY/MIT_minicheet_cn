...
void Simulation::sendControlParameter(const std::string& name, ControlParameterValue value, ControlParameterValueKind kind, bool isUser)
{
    ...
    // message data
    request.requestKind =
        isUser ? ControlParameterRequestKind::SET_USER_PARAM_BY_NAME : ControlParameterRequestKind::SET_ROBOT_PARAM_BY_NAME;
...
void Simulation::addCollisionPlane(
    double mu, double resti, double height, double sizeX, double sizeY, double checkerX, double checkerY, bool addToWindow)
{
 ...
 * @param addToWindow : if true, also adds graphics for the plane
 */
void Simulation::addCollisionBox(double mu, double resti, double depth, double width, double height, const Vec3<double>& pos,
    const Mat3<double>& ori, bool addToWindow, bool transparent)
{
