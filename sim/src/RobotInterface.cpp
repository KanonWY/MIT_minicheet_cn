...
void RobotInterface::handleVisualizationData(const lcm::ReceiveBuffer* rbuf, const std::string& chan, const cheetah_visualization_lcmt* msg)
{
    (void)rbuf;
    (void)chan;
    for (int i = 0; i < 3; i++)
...
void RobotInterface::sendControlParameter(const std::string& name, ControlParameterValue value, ControlParameterValueKind kind, bool isUser)
{
    if (_pendingControlParameterSend)
    {
        printf("[ERROR] trying to send control parameter while a send is in progress, "
               "ignoring!\n");
        return;
    }
    ...
    for (int iteration = 0; iteration < TIMES_TO_RESEND_CONTROL_PARAM;)
    {
        ...
        _parameter_request_lcmt.requestKind =
            isUser ? (s8)ControlParameterRequestKind::SET_USER_PARAM_BY_NAME : (s8)ControlParameterRequestKind::SET_ROBOT_PARAM_BY_NAME;
        ...
        else
        {
            _waitingForLcmResponse = false;
            // fail!
            printf("[RobotInterface] Failed to send parameter %s (iter %d timed out), "
                   "trying again...\n",
...
void RobotInterface::handleControlParameter(
    const lcm::ReceiveBuffer* rbuf, const std::string& chan, const control_parameter_respones_lcmt* msg)
{
    (void)rbuf;
    (void)chan;
    if (!_waitingForLcmResponse)
    {
        printf("[RobotInterface] Got a control parameter response when we weren't "
    ...
    if (msg->requestNumber == _parameter_request_lcmt.requestNumber && msg->parameterKind == _parameter_request_lcmt.parameterKind
